        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    <form method="POST" action="{{ route('admin.contatos.update', $contato) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="area" value="Área / Tipo de contato" />
                            <x-text-input id="area" name="area" type="text" class="mt-1 block w-full" :value="old('area', $contato->area)" placeholder="Ex: Contato Comercial, Suporte Técnico" required autofocus />
                            <x-input-error :messages="$errors->get('area')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="nome" value="Nome do responsável" />
                            <x-text-input id="nome" name="nome" type="text" class="mt-1 block w-full" :value="old('nome', $contato->nome)" required />
                        ...
                        <div>
                            ...
                            @if($contato->foto_url)
                                <div class="mt-1 flex items-center gap-4 flex-wrap">
                                    <img src="{{ asset('storage/' . $contato->foto_url) }}" alt="{{ $contato->nome }}" class="h-20 w-20 rounded-full object-cover border-2 border-gray-200 shadow">
                                    <label class="inline-flex items-center text-sm text-red-600">
                                        <input type="hidden" name="remover_foto" value="0">
                                        <input type="checkbox" name="remover_foto" value="1" class="rounded border-gray-300 text-red-600 focus:ring-red-500"> Remover foto
                                    </label>
                                </div>
                            @endif
                            <input x-ref="fotoInput" id="foto" name="foto" type="file" accept="image/*" @change="onFileChange($event)" class="mt-2 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100" />
                            <p class="mt-1 text-xs text-gray-500">Nova imagem: selecione e ajuste o recorte (quadrado, centralizado). Redimensionada para 400×400 px.</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="telefone" name="telefone" type="text" class="mt-1 block w-full" :value="old('telefone', $contato->telefone)" placeholder="(00) 00000-0000" />
                                <x-input-error :messages="$errors->get('telefone')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="email" value="E-mail (opcional)" />
                                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $contato->email)" placeholder="email@exemplo.com" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="icone" name="icone" type="text" class="mt-1 block w-full" :value="old('icone', $contato->icone)" placeholder="fa-solid fa-briefcase" />
                            ...
                            <div>
                                ...
                                <select id="cor" name="cor" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                                    @foreach(\App\Models\Contato::CORES as $valor => $label)
                                        <option value="{{ $valor }}" {{ old('cor', $contato->cor) == $valor ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="ordem" name="ordem" type="number" min="0" class="mt-1 block w-full" :value="old('ordem', $contato->ordem)" />
                                <x-input-error :messages="$errors->get('ordem')" class="mt-2" />
                            </div>
                            <div class="flex items-center pt-8">
                                <label class="inline-flex items-center">
                                    <input type="hidden" name="ativo" value="0">
                                    <input type="checkbox" name="ativo" value="1" {{ old('ativo', $contato->ativo) ? 'checked' : '' }} class="rounded border-gray-300 text-orange-600 shadow-sm focus:ring-orange-500">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t">
                            ...
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-semibold text-sm transition">
                                Salvar alterações
                            </button>
                        </div>
                    </form>
