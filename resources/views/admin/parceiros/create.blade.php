    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            tinymce.init({
                ...
                toolbar: 'undo redo | formatselect | bold italic underline | forecolor backcolor | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link emoticons | removeformat | code',
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    <form method="POST" action="{{ route('admin.parceiros.store') }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="descricao" name="descricao" rows="8" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500">{{ old('descricao') }}</textarea>
                                <p class="mt-1 text-xs text-gray-500">Use o editor para listas, negrito, itálico, emojis e links. Conteúdo formatado completo.</p>
                                <x-input-error :messages="$errors->get('descricao')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="site_url" value="Site (URL)" />
                                <x-text-input id="site_url" name="site_url" type="url" class="mt-1 block w-full" :value="old('site_url')" placeholder="https://..." />
                                <x-input-error :messages="$errors->get('site_url')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="instagram" value="Rede Social (Instagram)" />
                                <x-text-input id="instagram" name="instagram" type="url" class="mt-1 block w-full" :value="old('instagram')" placeholder="https://instagram.com/usuario" />
                            ...
                            <div class="md:col-span-2 border-t pt-6">
                                <label class="flex items-center">
                                    ...
                                    <input type="checkbox" name="ativo" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-orange-500" {{ old('ativo', true) ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>Salvar Parceiro</x-primary-button>
