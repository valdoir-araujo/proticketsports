...
@section('title', 'Editar equipe - Proticketsports')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div class="bg-gradient-to-r from-orange-500 to-red-500 px-6 py-5">
                <h1 class="text-xl sm:text-2xl font-black text-white">Editar equipe</h1>
                <p class="text-orange-100 text-sm mt-1">{{ $equipe->nome }}</p>
            </div>
            ...
            <div class="p-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('equipes.update', $equipe) }}" enctype="multipart/form-data" class="space-y-6"
                      x-data="{
                          estados: {{ $estados->toJson() }},
                          cidades: [],
                          estadoSelecionado: '{{ old('estado_id', $equipe->estado_id) }}',
                          cidadeSelecionada: '{{ old('cidade_id', $equipe->cidade_id) }}',
                          getCidades() {
                    ...
                    <div>
                        ...
                        <input id="nome" name="nome" type="text" value="{{ old('nome', $equipe->nome) }}" required autofocus
                    ...
                    <div>
                        ...
                        <select id="coordenador_id" name="coordenador_id" required
                            ...
                            @foreach($atletas as $atleta)
                                <option value="{{ $atleta->id }}" {{ old('coordenador_id', $equipe->coordenador_id) == $atleta->id ? 'selected' : '' }}>{{ $atleta->user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="data_fundacao" name="data_fundacao" type="date" value="{{ old('data_fundacao', $equipe->data_fundacao ? $equipe->data_fundacao->format('Y-m-d') : '') }}"
                                class="w-full rounded-lg border-2 border-slate-300 bg-white py-2.5 px-3 text-slate-900 focus:border-orange-500 focus:ring-2 focus:ring-orange-200">
                        </div>
                        <div>
                            ...
                            @if($equipe->logo_url)
                                ...
                                <img src="{{ asset('storage/' . $equipe->logo_url) }}" alt="" class="mb-2 rounded-lg max-h-16">
                            @endif
                    ...
                    <div class="flex flex-wrap items-center gap-4 pt-2">
                        <button type="submit" class="px-6 py-2.5 bg-orange-500 text-white font-bold rounded-lg hover:bg-orange-600 shadow-lg">
                            <i class="fa-solid fa-check mr-1"></i> Atualizar equipe
                        </button>
