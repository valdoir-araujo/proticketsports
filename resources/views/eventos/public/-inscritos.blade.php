            ...
            <div class="max-w-6xl mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                    ...
                    <div class="md:col-span-2 text-center md:text-left">
                        <p class="text-orange-400 font-semibold text-sm uppercase tracking-wider">Lista de Inscritos</p>
                        <h1 class="text-2xl md:text-3xl font-extrabold text-white mt-1 leading-tight">{{ $evento->nome }}</h1>
                        <a href="{{ route('eventos.public.show', $evento) }}" class="mt-3 inline-flex items-center gap-x-2 text-sm font-semibold text-slate-300 hover:text-white transition-colors">
                    ...
                    <div class="md:col-span-1 flex justify-center md:justify-end">
                        <div class="bg-white/10 backdrop-blur-md rounded-xl p-4 flex items-center gap-x-6 text-white text-center">
                            <div>
                                <span class="text-3xl font-bold">{{ count($inscricoes) }}</span>
                                <p class="text-xs uppercase tracking-wider text-slate-300">Inscritos</p>
                            </div>
                            <div class="border-l border-white/20 h-10"></div>
                            <div>
                                <span class="text-3xl font-bold">{{ $evento->data_evento->format('d') }}</span>
                                <p class="text-xs uppercase tracking-wider text-slate-300">{{ $evento->data_evento->translatedFormat('M') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div x-data="{
            ...
            <div class="container mx-auto px-4">
                 <div class="max-w-6xl mx-auto">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2 md:mb-4">
                        ...
                        <div class="flex p-1 bg-slate-100/80 rounded-lg w-full md:w-auto border border-slate-200">
                            <button @click="activeTab = 'atletas'" 
                                    :class="activeTab === 'atletas' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
                                    class="flex-1 md:flex-none px-6 py-2 rounded-md font-semibold text-sm transition-all flex items-center justify-center gap-2">
                                <i class="fa-solid fa-user-tag"></i> Por Atleta
                            </button>
                            <button @click="activeTab = 'equipes'" 
                                    :class="activeTab === 'equipes' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
                                    class="flex-1 md:flex-none px-6 py-2 rounded-md font-semibold text-sm transition-all flex items-center justify-center gap-2">
                                <i class="fa-solid fa-users"></i> Por Equipe
                            </button>
                        </div>
                        ...
                        <div class="md:hidden w-full flex justify-end">
                            <button @click="showFilters = !showFilters" 
                                    class="flex items-center gap-2 text-sm font-semibold text-orange-600 p-2 rounded-md hover:bg-orange-50 transition-colors border border-orange-100 w-full justify-center">
                                <i class="fa-solid" :class="showFilters ? 'fa-xmark' : 'fa-magnifying-glass'"></i>
                                <span x-text="showFilters ? 'Esconder Filtros' : 'Buscar / Filtrar'"></span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'atletas' && showFilters" 
                         ...
                         class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t md:border-none md:pt-0">
                        
                        <div class="relative">
                            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                <i class="fa-solid fa-tag text-gray-400"></i>
                            </div>
                            <select x-model="selectedCategory" class="block w-full rounded-md border-gray-300 pl-10 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" x-model.debounce.300ms="searchTerm" placeholder="Buscar por nome..." class="block w-full rounded-md border-gray-300 pl-10 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" x-model.debounce.300ms="searchCity" placeholder="Buscar por cidade..." class="block w-full rounded-md border-gray-300 pl-10 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" x-model.debounce.300ms="searchTeam" placeholder="Buscar por equipe..." class="block w-full rounded-md border-gray-300 pl-10 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'equipes' && showFilters" 
                         ...
                         x-transition:enter-start="opacity-0 -translate-y-2"
                         x-transition:enter-end="opacity-100 translate-y-0"
                         class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t md:border-none md:pt-0" style="display: none;">
                        
                        <div class="relative lg:col-span-2">
                            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                <i class="fa-solid fa-search text-gray-400"></i>
                            </div>
                            <input type="text" x-model.debounce.300ms="searchTeamTab" placeholder="Buscar nome da equipe..." class="block w-full rounded-md border-gray-300 pl-10 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                        </div>
                    </div>
        ...
        <div class="container mx-auto p-4 md:p-8">
            <div class="max-w-6xl mx-auto mt-8 min-h-[40vh]">
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="min-w-full table-fixed">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-20">Nº</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-4/12">Atleta</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-3/12">Cidade/UF</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-3/12">Equipe</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-2/12">Situação</th>
                                                </tr>
                                            </thead>
                ...
                {{-- CONTEÚDO DA ABA 2: RANKING DE EQUIPES (COM ACORDEÃO APRIMORADO E COMPACTO) --}}
                <div x-show="activeTab === 'equipes'" style="display: none;" class="space-y-6">
                    <div class="border-l-4 border-orange-500 pl-4">
                        <h2 class="text-3xl font-bold text-slate-800">Classificação por Tamanho da Equipe</h2>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full table-fixed">
                                <thead class="bg-gray-50 border-b">
                                    <tr>
                                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-16">Pos</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-4/12">Nome da Equipe</th>
                                        <th class="px-4 py-3 text-center text-xs font-bold text-green-600 uppercase tracking-wider w-2/12">Confirmados</th>
                                        <th class="px-4 py-3 text-center text-xs font-bold text-yellow-600 uppercase tracking-wider w-2/12">A Confirmar</th>
                                        <th class="px-4 py-3 text-center text-xs font-bold text-gray-700 uppercase tracking-wider w-2/12">Total</th>
                                ...
                                <template x-for="(equipe, index) in listEquipes" :key="equipe.nome">
                                    <tbody x-data="{ expanded: false }" class="border-b border-gray-100 last:border-b-0">
                                        ...
                                        <tr @click="expanded = !expanded" 
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-green-600 text-center" x-text="equipe.confirmados"></td>
                                            <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-yellow-600 text-center" x-text="equipe.pendentes"></td>
                                            
                                            <td class="px-4 py-4 whitespace-nowrap text-sm font-bold text-orange-600 text-center text-lg bg-orange-50/30" x-text="equipe.quantidade"></td>
